/*
  sendxevent.c --- Taken from the contrib folder of the lirc package (www.lirc.org)
                   for stand-alone use
...
#----------------------------------------------------------------------------
  ...
  2006/12/22:  Tiny tweaks by gradgrind:
                    - check for 0 arguments to avoid segfault
                    - simplify compilation hint (xorg doesn't need -L/usr/X11R6/lib)
  ...
  I compile with 'gcc -lX11 -o sendxevent sendxevent.c'
...
/* 
 * Local variables:
 * compile-command: "gcc -lX11 -o sendxevent sendxevent.c"
 * End:
 */
